(function() {
  ...
  exports.getSingle = function(req, res) {
    ...
    globals.modelsObject[modelName].find(request).success(function(modelInstance) {
      globals.helpers.getAllModelNames(function(err, modelsNames) {
        ...
        var getAllModelInstances = {
          get: function(_, callback) {
            globals.modelsObject[_.model].findAll().success(function(instances) {
            ...
            switch (associationData.type) {
              case "hasMany":
                require("async").map(globals.helpers.processMultipleSelectToArray(req.body["" + associationData.as]), retrieveAssociationObjects.retrieve.bind(retrieveAssociationObjects), function(err, retrievedObjects) {
                  modelInstance["set" + associationData.as](retrievedObjects).success(function() {
                    callback(null, "OK");
                  });
                });
              case "belongsTo":
                globals.modelsObject[associationData.model].find({
                  ...
                  modelInstance["set" + associationData.as](associationModel).success(function() {
                    callback(null, "OK");
                  });
                }).error(function(err) {
                  callback(err, null);
